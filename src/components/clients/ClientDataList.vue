      ...
      >
        <template v-slot:top>
          <div class="row justify-start full-width">
            ...
            <div class="col-4"><q-space /></div>
            <div class="col-1" @click="addItem"><v-icon name="add_circle" size="md" /> Add</div>
            <q-input v-model="filter" label="Filter" class="col-4"></q-input>
          </div>
        </template>
        <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            <v-icon name="edit" @click="editItem(props.row)" class="mx-md" />
            <v-icon name="delete" @click="deleteItem(props.row)" class="mx-md" />
          </q-td>
        </template>
      </v-data-table>
    </div>
...
const deleteItem = async (item) => {
  const resp = await myClientStore.deleteData(ccd.getDeleteUrl(myClientStore.SectionName), item.id)
